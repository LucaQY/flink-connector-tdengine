...
public abstract class AbstractJdbcRowConverter implements JdbcRowConverter {
     ...
     * @throws SQLException
     */
    private Object getTimestampField(ResultSet resultSet)throws SQLException {
        ...
        switch (timestampFormat) {
            case TIMESTAMP:
                return resultSet.getLong(tsField);
            case UTC:
                return resultSet.getTimestamp(tsField);
            default:
                return resultSet.getLong(tsField);
        }
    }
    ...
    public RowData toInternal(ResultSet resultSet) throws SQLException {
        ...
        for (int pos = 0; pos < rowType.getFieldCount(); pos++) {
            ...
            // 由于tdengine首个字段必须为时间戳
            // 所以才能这样写死
            Object field;
            if (pos == 0) {
                field = getTimestampField(resultSet);
            } else {
